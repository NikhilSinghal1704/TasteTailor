...
def signup(request):
    ...
    if request.method == "POST":
        ...
        if User.objects.filter(username = username):
            return render(request, 'signup.html', {'umessage': "Username already exist"})
        
        elif is_valid_string(username) == False:
            return render(request, 'signup.html', {'umessage': "Required. 150 characters or fewer. Letters, digits and @/./+/-/_ only"})
            ...
            messages.error(request, "Password should contain 8 characters, at least one alphabetic character, at least one special character, and at least one number.")
            return render(request, 'signup.html', {'pmessage': "Invalid Password"})
        
        elif pass1 != pass2:
            return render(request, 'signup.html', {'pmessage': "Password doesn't match"})
